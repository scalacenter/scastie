...
object ShareModal {
  def apply(
    ...
    snippetId: SnippetId,
    isClosed: Boolean,
    close: Callback) = new ShareModal(router, snippetId, isClosed, close).build

  private class ShareModal(
    router: RouterCtl[Page],
    snippetId: SnippetId,
    isClosed: Boolean,
    close: Callback){
      ...
      ScalaComponent
        ...
        .render_P {
          case (router, snippetId, isClosed, close) =>            
            ...
            def copyLink(e: ReactEventFromInput): Callback = Callback {
            ...
            Modal(
              ...
              TagMod(
                p(clazz := "modal-intro")("Copy and share your code snippet's URL:"),
