  ...
  private def parsePo(poContent: String): Map[String, String] = {
    ...
    for (line <- poContent.linesIterator) {
      line.trim match {
        ...
        case quotedLine(text) if collectingMsgid                             => msgidBuffer.append(text)
        case quotedLine(text) if collectingMsgstr                            => msgstrBuffer.append(text)
