...
class EnsimeActor(system: ActorSystem, sbtRunner: ActorSelection)
  ...
  private val ensimeDir: Path = Files.createTempDirectory("scastie-ensime")
  ...
  private val codeFile: Path = ensimeDir.resolve("src/main/scala/main.scala")
  Files.createDirectories(codeFile.getParent)
  write(codeFile, Inputs.defaultCode)
  ...
  override def preStart(): Unit = {
    ...
    sbtRunner ! CreateEnsimeConfigRequest(currentConfig)
    super.preStart()
  }

  private def startEnsimeServer(config: String) = {
    val ensimeConfigFile = ensimeDir.resolve(".ensime")
    write(ensimeConfigFile, config)

    val ensimeCacheDir = ensimeDir.resolve(".ensime_cache")
    ...
    ensimeProcess = Some(
      ...
      ).directory(ensimeDir.toFile).start()
    )
  ...
  override def receive: Receive = {
    case EnsimeConfigResponse(sbtDir: String, config: String) =>
        ...
        startEnsimeServer(
          config
            .replaceAll(
              ...
              ensimeDir.toString)
            .replaceAll(
              Paths.get(sbtDir).getFileName.toString,
              ensimeDir.getFileName.toString
            )
        )
      } catch {
  ...
  private def processRequest(
    ...
    if (serverState.isReady) {
      ...
      log.info(
        s"Not ready to process request $taskId"
      )
    }
  }
