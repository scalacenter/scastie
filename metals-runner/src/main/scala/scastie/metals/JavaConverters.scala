...
object JavaConverters {
  ...
  extension (documentation: JEither[String, MarkupContent])
    def toDocstring =
      documentation.asScala.map(_.getValue).merge

  extension (documentation: JEither[String, MarkedString])
    def toHoverContent =
      documentation.asScala.map(_.getValue).merge

  extension (item: CompletionItem)
    def getDocstring = Option(item.getDocumentation).fold("")(_.toDocstring)
  ...
  extension (completions: CompletionList)
    def toSimpleScalaList: Set[CompletionItemDTO] =
      (for {
        ...
        CompletionItemDTO(filterText, detail, kind, order, insertInstructions, additionalInsertInstructions, data)
      ).toSet
  ...
  def createInsertInstructions(completion: CompletionItem): Option[InsertInstructions] = {
    completion.getTextEdit().asScala match {
      case Left(textEdit) =>
        ...
        Some(InsertInstructions(textEdit.getNewText.replace("$0", ""), if move > 0 then move else textEdit.getNewText.length))
      case Right(insertReplace) =>
        println("[InsertReplaceEdit] should not appear as it is not used in metals")
        None

    }
  }

  def createAdditionalInsertInstructions(completion: CompletionItem): List[AdditionalInsertInstructions] =
    Option(completion.getAdditionalTextEdits()).map(_.asScala).toList.flatten.map(edit => {
      val range = edit.getRange
      val start = range.getStart
      val end = range.getEnd
      println(edit.getNewText)
      AdditionalInsertInstructions(edit.getNewText(), start.getLine + 1, start.getCharacter, end.getLine + 1, end.getCharacter)
    }).toList
