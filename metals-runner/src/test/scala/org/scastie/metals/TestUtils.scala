...
object TestUtils extends Assertions with CatsEffectAssertions {
  ...
  val testTargets = List(BuildInfo.latestLTS, BuildInfo.stableLTS, BuildInfo.latestNext).map(Scala3.apply) ++
  ...
  def testCompletionEdit(
    ...
    val newText = server.complete(request).map(_.items.head).map { completionDTO =>
      ...
      request.offsetParams.content
        .patch(offset, completionDTO.instructions.text, editRange.endChar - editRange.startChar)
    }
