...
object ApplicationBuild extends Build {
  ...
  val rendererWorker =
    Project(id = "rendererWorker", base = file(".")).settings(
  ...
  private def runAllTask(discoveredMainClasses: Seq[String],
  ...
  private def addDepsToState(state: State): State = {
    def extractDependencies(eval: compiler.Eval,
                            loader: ClassLoader,
                            state: State): Seq[Setting[_]] = {
    ...
    val settings = extractDependencies(
      state.get(Keys.sessionSettings).get.currentEval(),
      extracted.currentLoader,
      state)
    val sessionSettings = sbt.Shim.SettingCompletions
      .setThis(state, extracted, settings, "")
      .session
    BuiltinCommands.reapply(
      sessionSettings.appendRaw(onLoad in Global := idFun),
      extracted.structure,
      state)
  }
}
