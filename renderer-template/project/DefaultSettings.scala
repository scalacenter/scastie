...
object DefaultSettings {
  ...
  def scalacOptions: Def.Setting[Task[Seq[String]]] = {
    Keys.scalacOptions <++= (scalaSource in Compile, baseDirectory, scalaVersion) map {
      (scalaSource, baseDirectory, scalaVersion) =>
        ...
        List("-deprecation", "-unchecked", "-Xcheckinit") ++ sxrOptions(scalaVersion, baseDirectory) ++ featureOptions ++ warnAll
    }
  }
  ...
  class IsStartsWith(startsWith: String) {
    ...
    def unapply(scalaVersion: String): Option[String] = Some(scalaVersion).filter(apply)
  }
  ...
  def addSupportedCompilerPlugin(version: PartialFunction[String, ModuleID]): Def.Setting[Seq[ModuleID]] =
