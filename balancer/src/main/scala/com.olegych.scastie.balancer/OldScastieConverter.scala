...
object OldScastieConverter {
  ...
  def convertOldInput(content: String): Inputs = {
    ...
    if(blockStartPos != -1 && blockEndPos != -1 && blockEndPos > blockStartPos) {
      ...
      val converter = 
        sbtConfig.split(nl).foldLeft(Converter.nil){ case (converter, line) =>
      ...
      val scalaTarget = 
        targetType match {
          ...
          case Some(ScalaTargetType.Typelevel) => 
            scalaVersion.map(sv => ScalaTarget.Typelevel(sv)).getOrElse(
              ScalaTarget.Typelevel.default
            )

          case _ =>
            scalaVersion.map(sv => ScalaTarget.Jvm(sv)).getOrElse(
