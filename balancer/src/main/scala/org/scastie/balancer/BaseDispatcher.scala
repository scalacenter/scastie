...
abstract class BaseDispatcher[R, S](config: Config) extends Actor with ActorLogging {
  ...
  def getRemoteServers(
    ...
    runnerName: String,
    actorName: String
  ): TrieMap[SocketAddress, ActorSelection] = {
    getRemoteActorsPath(key, runnerName, actorName).map { case (address, url) =>
      (address, connectRunner(url))
    }
  }
