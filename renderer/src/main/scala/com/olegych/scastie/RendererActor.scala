  ...
  def receive = LoggingReceive {
    killer {
      case paste @ Paste(id,
        ...
        if (sbt.isEmpty) {
          sbt = blocking {
            Option(RendererTemplate.create(sbtDir.root, generateId))
          }
        }
        ...
        sbt.foreach { sbt =>
          ...
          applyRunKiller(paste) {
            if (scalaTargetType == ScalaTargetType.JVM ||
              ...
              sbt.process(";compile ;run-all",
                          processSbtOutput(sender,
                                           id,
                                           withInstrumentations = true))
