...
class OAuthRoutesImpl(implicit ec: ExecutionContext) {
  ...
  val loginRoute = OAuthEndpoints.login.serverLogicSuccess(maybeReferer => {
    val referer = maybeReferer.getOrElse("/")
    Future(s"$authorizationUrl?client_id=$clientId&state=$referer")
  })

  val logoutRoute = OAuthEndpoints.logout.serverLogicSuccess(_ => {
    Future {
  ...
  def loginGithubRoute(backend: SttpBackend[Future, Any]) = OAuthEndpoints.loginGithub.serverLogic(queryParameters => {
  ...
  private def fetchUser(accessToken: GithubAccessToken): Future[Option[User]] = basicRequest
    .response(asStringAlways)
