...
object Deployment {
  ...
  def deployServerTask(server: Project, sbtRunner: Project): Def.Initialize[Task[Unit]] = Def.task {
  ...
  def deployTask(server: Project, sbtRunner: Project, metalsRunner: Project): Def.Initialize[Task[Unit]] = Def.task {
    val deployment      = deploymentTask(sbtRunner).value
    val serverZip       = (server / Universal / packageBin).value.toPath
  ...
  private def serverZipTask(genericServer: Project): Def.Initialize[Task[Path]] = Def.task {
...
class Deployment(rootFolder: File, version: String, sbtDockerImage: ImageName, val logger: Logger) {
  ...
  def deployLocal(serverZip: Path, metalsServerZip: Path): Unit = {
    ...
    val runnerScriptContent = s"""|#!/usr/bin/env bash
                                  |
                                  |docker run \\
                                  |  --network=host \\
                                  |  -e RUNNER_PORT=5150 \\
  ...
  def deployMetalsRunner(metalsRunnerZip: Path): Unit = {
    ...
    val deploymentFiles = deployMetalsRunnerFiles(metalsRunnerZip, metalsRunnerScriptDir, local = false)
  ...
  def killRunners(): Unit = {
    ...
    val killScriptContent = """|#!/usr/bin/env bash
                               |
    ...
    val proxyScriptContent = s"""|rm kill-proxy.sh
  ...
  def deployRunners(runner: String, image: String, runnersPortsStart: Int, runnersPortsSize: Int): Unit = {
    ...
    val proxyScriptContent = s"""|rm ${runner}-proxy.sh
                                 |rsync $scriptFileName $runnerUri:$scriptFileName
                                 |ssh $runnerUri ./$scriptFileName
                                 |rm $scriptFileName""".stripMargin
  ...
  private def rsyncServer(file: Path) = rsync(file, userName, serverHostname, logger)
