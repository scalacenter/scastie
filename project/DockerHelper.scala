...
object DockerHelper {
  ...
  def scalaCliRunner(baseDirectory: Path,
            scalaCliTargetDir: Path,
            ivyHome: Path,
            organization: String,
            artifact: Path,
          ): Dockerfile = {
    ...
    val ivyLocalTemp = scalaCliTargetDir.resolve("ivy")
    ...
    val containerUsername = "scalaCliRunnerContainer"
    ...
    new Dockerfile {
      ...
      runRaw(s"wget -q -O scala-cli.gz  https://github.com/Virtuslab/scala-cli/releases/latest/download/scala-cli-x86_64-pc-linux-static.gz && gunzip scala-cli.gz")
      runRaw(s"chmod +x scala-cli")
      runRaw(s"mv scala-cli /usr/bin/")

      runRaw(s"addgroup -g 433 $containerUsername")
      runRaw(s"adduser -h $userHome -G $containerUsername -D -u 433 -s /bin/sh $containerUsername")
      ...
      runRaw(s"""scala-cli -e "println(1 + 1)"""")
      runRaw(s"""scala-cli -e "println(1 + 1)"""")
